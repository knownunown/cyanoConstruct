...
class NamedSequenceDB(db.Model):
    ...
    def getHTMLdisplay(self):
        ...
        retArray.append(
            """<div class = "hideableTitle nameTitle" id = "{libraryName}{NSname}">
        ...
        for comp in allComps:
            retArray.append(
                """<div class = "hideableTitle compTitle" id = "{libraryName}{compNameID}">
                                                        <input class = "titleLeft subtleButton" type = "button"
    ...
    def __eq__(self, other):
        return (self.getType() == other.getType()) and (
            self.getNameID() == other.getNameID()
        )

    # self != other
    def __ne__(self, other):
        return (self.getType() != other.getType()) or (
            self.getNameID() != other.getNameID()
        )
...
class SpacerDataDB(db.Model):
    ...
    def __eq__(self, other):
        return (self.getPosition() == other.getPosition()) and (
            self.getTerminalLetter() == other.getTerminalLetter()
        )

    def __ne__(self, other):
        return (self.getPosition() != other.getPosition()) or (
            self.getTerminalLetter() != other.getTerminalLetter()
        )
    ...
    def __lt__(self, other):
        if self.getPosition() == other.getPosition():
            letterOrder = ["S", "M", "T", "L"]
            return letterOrder.index(self.getTerminalLetter()) < letterOrder.index(
                other.getTerminalLetter()
            )
        else:
...
class ComponentDB(db.Model):
    ...
    def __repr__(self):
        return "<Component {id} {nameID} {name}>".format(
            id=self.getID(), nameID=self.getNameID(), name=self.getName()
        )

    def __str__(self):
        return "Component {nameID}: {name}".format(
            nameID=self.getNameID(), name=self.getName()
        )
    ...
    def getSeqHTML(self):
        ...
        HTML unsafe."""
        retStr = (
            "<span class = 'enzymeSeq'>{enzymeLeft}</span>"
            ...
            "<span class = 'enzymeSeq'>{enzymeRight}</span>"
        ).format(
    ...
    def getLongName(self):
        retStr = "{type} {name} Position: {pos}".format(
            type=self.getType(), name=self.getName(), position=self.getPosition()
        )
    ...
    def getCompZIP(self, offset):
        ...
        retDict[idStr + "-CompleteSequence.fasta"] = (
            ">" + idStrAndName + " complete sequence\n" + self.getFullSeq()
        )
        retDict[idStr + "-LeftPrimer.fasta"] = (
            ">" + idStrAndName + " left primer\n" + self.getLeftPrimer()
        )
        retDict[idStr + "-RightPrimer.fasta"] = (
            ">" + idStrAndName + " right primer\n" + self.getRightPrimer()
        )
    ...
    def getGenBankFile(self, offset):
        ...
        lenTotal = (
            lenSpacerL + lenSpacerR + lenNNL + lenNNR + lenEnzymeL + lenEnzymeR + lenSeq
        )
        ...
        # add the header
        fileByLines.append(
            "LOCUS\t\t{nameID}\t{length} bp\tDNA\tlinear\t{date}".format(
                nameID=self.getNameID(), length=lenTotal, date=date
            )
        )
        fileByLines.append(
            "DEFINITION\t{nameID} ({name}) component from CyanoConstruct.".format(
                nameID=self.getNameID(), name=self.getName()
            )
        )
        ...
        fileByLines.append(
            "\t\t\t/ApEinfo_fwdcolor=#CFEC67"
        )  # this is only for benchling
        ...
        if self.getType() == "RBS":
            fileByLines.append("\tmisc_feature\t" + str(i + 1) + ".." + str(i + 2))
            fileByLines.append('\t\t\t/note="RBS padding"')
            fileByLines.append("\t\t\t/ApEinfo_fwdcolor=#E6855F")
            fileByLines.append("\t\t\t/ApEinfo_revcolor=#E6855F")
            i += 2  # length of GG padding
        ...
        while i < (len(seq) // 60):
            i60 = i * 60
            line = (
                "{number} {block1} {block2} {block3} {block4} {block5} {block6}".format(
                    **{
    ...
    # named sequence info.
    def getNamedSequence(self) -> NamedSequenceDB:
    ...
    def getRightSpacer(self):
        return (
            "GG" if self.getType() == "RBS" else ""
        ) + self.getSpacerData().getSpacerRight()  # what is this naming
    ...
    def getFullSpacerRight(self):
        return (
            "GG" if self.getType() == "RBS" else ""
        ) + self.getSpacerData().getFullSeqRight()
...
class BackboneDB(db.Model):
    ...
    name = db.Column(db.String(20))
    desc = db.Column(
        db.String(128), nullable=False, default="No description."
    )  # Need to mark as unsafe for HTML
    ...
    features = db.Column(
        db.Text(), nullable=False, default="FEATURES\t\t\tLocation/Qualifiers"
    )
    ...
    def getHTMLdisplay(self):
        ...
        retArray.append(
            """<div class = "hideableTitle nameTitle" id = "{libraryName}{BBname}Backbone">
                                                        <input class = "titleLeft subtleButton" type = "button"
        ...
        if not isDefault:
            retArray.append(
                """<br><hr><input type = "button" class = "styledButton"
